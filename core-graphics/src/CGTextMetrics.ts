import { CGTextStyle, TextStyleWhiteSpace } from './CGTextStyle';
...
export class CGTextMetrics
{
     ...
     */
    constructor(text: string, style: CGTextStyle, width: number, height: number, lines: string[], lineWidths: number[],
     ...
     */
    public static measureText(text: string, style: CGTextStyle, wordWrap: boolean, canvas = CGTextMetrics._canvas): CGTextMetrics
    {
        wordWrap = (wordWrap === undefined || wordWrap === null) ? style.wordWrap : wordWrap;
        const font = style.toFontString();
        const fontProperties = CGTextMetrics.measureFont(font);
        ...
        const outputText = wordWrap ? CGTextMetrics.wordWrap(text, style, canvas) : text;
     ...
     */
    private static wordWrap(text: string, style: CGTextStyle, canvas = CGTextMetrics._canvas): string
    {
        const context = canvas.getContext('2d')!;
        ...
        const collapseSpaces = CGTextMetrics.collapseSpaces(whiteSpace);
        const collapseNewlines = CGTextMetrics.collapseNewlines(whiteSpace);
        ...
        const tokens = CGTextMetrics.tokenize(text);
        ...
        for (let i = 0; i < tokens.length; i++)
        {
            ...
            if (collapseSpaces)
            {
                ...
                const currIsBreakingSpace = CGTextMetrics.isBreakingSpace(token);
                const lastIsBreakingSpace = CGTextMetrics.isBreakingSpace(line[line.length - 1]);
            ...
            const tokenWidth = CGTextMetrics.getFromCache(token, letterSpacing, cache, context);
            ...
            if (tokenWidth > wordWrapWidth)
            {
                ...
                if (CGTextMetrics.canBreakWords(token, style.breakWords))
                {
                    ...
                    const characters = CGTextMetrics.wordWrapSplit(token);
                    ...
                    for (let j = 0; j < characters.length; j++)
                    {
                        ...
                        const characterWidth = CGTextMetrics.getFromCache(char, letterSpacing, cache, context);
    ...
    public static measureFont(font: string): IFontMetrics
    {
        ...
        const canvas = CGTextMetrics._canvas;
        const context = CGTextMetrics._context;

        context.font = font;

        const metricsString = CGTextMetrics.METRICS_STRING + CGTextMetrics.BASELINE_SYMBOL;
        const width = Math.ceil(context.measureText(metricsString).width);
        let baseline = Math.ceil(context.measureText(CGTextMetrics.BASELINE_SYMBOL).width);
