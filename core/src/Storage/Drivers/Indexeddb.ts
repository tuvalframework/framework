...
function _checkBlobSupportWithoutCaching(idb) {
    return new Promise(function(resolve) {
        ...
        var blob = StorageUtils.CreateBlob(['']);
...
function _decodeBlob(encodedBlob) {
    ...
    return StorageUtils.CreateBlob([arrayBuff], { type: encodedBlob.type });
}
...
function _initStorage(options) {
    ...
    var dbContext = dbContexts[(dbInfo as any).name];

    // ...or create a new context.
    if (!dbContext) {
        dbContext = createDbContext();
        // Register the new context in the global container.
        dbContexts[(dbInfo as any).name] = dbContext;
    }
    ...
    for (var j = 0; j < dbContext.forages.length; j++) {
        var forage: any = dbContext.forages[j];
        if (forage !== self) {
...
function getItem(key, callback) {
    ...
    key = StorageUtils.NormalizeKey(key);
   ...
   StorageUtils.ExecuteCallback(promise, callback);
    return promise;
}
...
function dropInstance(options, callback) {
    callback = StorageUtils.GetCallback.apply(this, arguments as any);
