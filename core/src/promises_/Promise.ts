...
function handleResolutionMethods(
    ...
    catch (ex) { if (targetReject) targetReject(ex); }
}
...
export class PromiseState<T> extends DisposableBase {
...
export abstract class PromiseBase<T> extends PromiseState<T> implements PromiseLike<T>, Promise<T>
{
    ...
    then<TFulfilled = T, TRejected = never>(onFulfilled: TSDNPromise.Fulfill<T, TFulfilled> | undefined | null, onRejected?: TSDNPromise.Reject<TRejected> | undefined | null): any //PromiseBase<TFulfilled | TRejected>
    {
        this.throwIfDisposed();
     ...
     */
    'catch'<TResult = never>(onRejected: TSDNPromise.Reject<TResult>): any //PromiseBase<T | TResult>
    {
    ...
    thenSynchronous<TFulfilled = T, TRejected = never>(
        ...
        try {
            switch (this.state) {
                case TSDNPromise.State.Fulfilled:
                    return onFulfilled
                        ? resolve(this._result as any, onFulfilled as any, TSDNPromise.resolve)
 ...
 */
export class TSDNPromise<T> extends Resolvable<T>
{
    ...
    protected _resolvedCalled: boolean = false;
    ...
    extends TSDNPromise<T[]>
{
    ...
    map<U>(transform: (value: T) => U): ArrayPromise<U> {
        ...
        return new ArrayPromise<U>(resolve => {
            this.doneNow((result: T[] | undefined) => resolve(result!.map(transform)));
        }, true);
...
export class PromiseCollection<T>
    ...
    map<U>(transform: (value: T) => U): ArrayPromise<U> {
        ...
        return new ArrayPromise<U>(resolve => {
            this.all()
                .doneNow((result: T[] | undefined) => resolve(result!.map(transform)));
        }, true);
    }
    ...
    reduce<U>(
        reduction: (previousValue: U, currentValue: T, i?: number, array?: PromiseLike<T>[]) => U,
        initialValue?: U | PromiseLike<U>): PromiseBase<U> {
        // throw new NotImplementedException('reduce');
        this.throwIfDisposed();
            ...
            .reduce(
                (
                    ...
                    handleSyncIfPossible(previous,
                        (p: U) => handleSyncIfPossible(current, (c: T) => reduction(p, c, i, array)) as any),
