...
export function localeEras(m, format) {
    ...
    for (i = 0, l = eras.length; i < l; ++i) {
        switch (typeof eras[i].since) {
            case 'string':
                // truncate time
                date = (moment as any)(eras[i].since).startOf('day');
        ...
        switch (typeof eras[i].until) {
            ...
            case 'string':
                // truncate time
                date = (moment as any)(eras[i].until).startOf('day').valueOf();
...
export function localeErasConvertYear(era, year) {
    ...
    if (year === undefined) {
        return (moment as any)(era.since).year();
    } else {
        return (moment as any)(era.since).year() + (year - era.offset) * dir;
    }
}
...
export function getEraYear() {
    ...
    for (i = 0, l = eras.length; i < l; ++i) {
        ...
        if (
            ...
            return (
                (this.year() - (moment as any)(eras[i].since).year()) * dir +
                eras[i].offset
            );
        }
    }
