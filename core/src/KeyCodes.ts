import { int } from "./float";

export class KeyCodes {
    /**
     * Key code for A
     */
    public static readonly KEY_A = 65;
    /**
     * Key code for B
     */
    public static readonly KEY_B = 66;
    /**
     * Key code for C
     */
    public static readonly KEY_C = 67;
    /**
     * Key code for D
     */
    public static readonly KEY_D = 68;
    /**
     * Key code for E
     */
    public static readonly KEY_E = 69;
    /**
     * Key code for F
     */
    public static readonly KEY_F = 70;
    /**
     * Key code for G
     */
    public static readonly KEY_G = 71;
    /**
     * Key code for H
     */
    public static readonly KEY_H = 72;
    /**
     * Key code for I
     */
    public static readonly KEY_I = 73;
    /**
     * Key code for J
     */
    public static readonly KEY_J = 74;
    /**
     * Key code for K
     */
    public static readonly KEY_K = 75;
    /**
     * Key code for L
     */
    public static readonly KEY_L = 76;
    /**
     * Key code for M
     */
    public static readonly KEY_M = 77;
    /**
     * Key code for N
     */
    public static readonly KEY_N = 78;
    /**
     * Key code for O
     */
    public static readonly KEY_O = 79;
    /**
     * Key code for P
     */
    public static readonly KEY_P = 80;
    /**
     * Key code for Q
     */
    public static readonly KEY_Q = 81;
    /**
     * Key code for R
     */
    public static readonly KEY_R = 82;
    /**
     * Key code for S
     */
    public static readonly KEY_S = 83;
    /**
     * Key code for T
     */
    public static readonly KEY_T = 84;
    /**
     * Key code for U
     */
    public static readonly KEY_U = 85;
    /**
     * Key code for V
     */
    public static readonly KEY_V = 86;
    /**
     * Key code for W
     */
    public static readonly KEY_W = 87;
    /**
     * Key code for X
     */
    public static readonly KEY_X = 88;
    /**
     * Key code for Y
     */
    public static readonly KEY_Y = 89;
    /**
     * Key code for Z
     */
    public static readonly KEY_Z = 90;
    /**
     * Key code number 0
     */
    public static readonly KEY_ZERO = 48;
    /**
     * Key code number 1
     */
    public static readonly KEY_ONE = 49;
    /**
     * Key code number 2
     */
    public static readonly KEY_TWO = 50;
    /**
     * Key code number 3
     */
    public static readonly KEY_THREE = 51;
    /**
     * Key code number 4
     */
    public static readonly KEY_FOUR = 52;
    /**
     * Key code number 5
     */
    public static readonly KEY_FIVE = 53;
    /**
     * Key code number 6
     */
    public static readonly KEY_SIX = 54;
    /**
     * Key code number 7
     */
    public static readonly KEY_SEVEN = 55;
    /**
     * Key code number 8
     */
    public static readonly KEY_EIGHT = 56;
    /**
     * Key code number 9
     */
    public static readonly KEY_NINE = 57;
    /**
     * Key code for number 0 on numeric keyboard
     */
    public static readonly KEY_NUM_ZERO = 96;
    /**
     * Key code for number 1 on numeric keyboard
     */
    public static readonly KEY_NUM_ONE = 97;
    /**
     * Key code for number 2 on numeric keyboard
     */
    public static readonly KEY_NUM_TWO = 98;
    /**
     * Key code for number 3 on numeric keyboard
     */
    public static readonly KEY_NUM_THREE = 99;
    /**
     * Key code for number 4 on numeric keyboard
     */
    public static readonly KEY_NUM_FOUR = 100;
    /**
     * Key code for number 5 on numeric keyboard
     */
    public static readonly KEY_NUM_FIVE = 101;
    /**
     * Key code for number 6 on numeric keyboard
     */
    public static readonly KEY_NUM_SIX = 102;
    /**
     * Key code for number 7 on numeric keyboard
     */
    public static readonly KEY_NUM_SEVEN = 103;
    /**
     * Key code for number 8 on numeric keyboard
     */
    public static readonly KEY_NUM_EIGHT = 104;
    /**
     * Key code for number 9 on numeric keyboard
     */
    public static readonly KEY_NUM_NINE = 105;
    /**
     * Key code for multiply on numeric keyboard
     */
    public static readonly KEY_NUM_MULTIPLY = 106;
    /**
     * Key code for plus on numeric keyboard
     */
    public static readonly KEY_NUM_PLUS = 107;
    /**
     * Key code for minus on numeric keyboard
     */
    public static readonly KEY_NUM_MINUS = 109;
    /**
     * Key code for period on numeric keyboard
     */
    public static readonly KEY_NUM_PERIOD = 110;
    /**
     * Key code for division on numeric keyboard
     */
    public static readonly KEY_NUM_DIVISION = 111;
    /**
     * Alt key code.
     */
    public static readonly KEY_ALT = 18;
    /**
     * Backspace key code.
     */
    public static readonly KEY_BACKSPACE = 8;
    /**
     * Control key code.
     */
    public static readonly KEY_CTRL = 17;
    /**
     * Delete key code (also numeric keypad delete).
     */
    public static readonly KEY_DELETE = 46;
    /**
     * Down arrow code (Also numeric keypad down).
     */
    public static readonly KEY_DOWN = 40;
    /**
     * End key code (Also numeric keypad south west).
     */
    public static readonly KEY_END = 35;
    /**
     * Enter key code.
     */
    public static readonly KEY_ENTER = 13;
    /**
     * Escape key code.
     */
    public static readonly KEY_ESCAPE = 27;
    /**
     * Home key code (Also numeric keypad north west).
     */
    public static readonly KEY_HOME = 36;
    /**
     * Left key code (Also numeric keypad west).
     */
    public static readonly KEY_LEFT = 37;
    /**
     * Page down key code (Also numeric keypad south east).
     */
    public static readonly KEY_PAGEDOWN = 34;
    /**
     * Page up key code (Also numeric keypad north east).
     */
    public static readonly KEY_PAGEUP = 33;
    /**
     * Right arrow key code (Also numeric keypad east).
     */
    public static readonly KEY_RIGHT = 39;
    /**
     * Shift key code.
     */
    public static readonly KEY_SHIFT = 16;
    /**
     * Tab key code.
     */
    public static readonly KEY_TAB = 9;
    /**
     * Up Arrow key code (Also numeric keypad north).
     */
    public static readonly KEY_UP = 38;
    /**
     * Key code for F1
     */
    public static readonly KEY_F1 = 112;
    /**
     * Key code for F2
     */
    public static readonly KEY_F2 = 113;
    /**
     * Key code for F3
     */
    public static readonly KEY_F3 = 114;
    /**
     * Key code for F4
     */
    public static readonly KEY_F4 = 115;
    /**
     * Key code for F5
     */
    public static readonly KEY_F5 = 116;
    /**
     * Key code for F6
     */
    public static readonly KEY_F6 = 117;
    /**
     * Key code for F7
     */
    public static readonly KEY_F7 = 118;
    /**
     * Key code for F8
     */
    public static readonly KEY_F8 = 119;
    /**
     * Key code for F9
     */
    public static readonly KEY_F9 = 120;
    /**
     * Key code for F10
     */
    public static readonly KEY_F10 = 121;
    /**
     * Key code for F11
     */
    public static readonly KEY_F11 = 122;
    /**
     * Key code for F12
     */
    public static readonly KEY_F12 = 123;
    /**
     * Key code for Windows key on Firefox Linux
     */
    public static readonly KEY_WIN_KEY_FF_LINUX = 0;
    /**
     * Key code for Mac enter key
     */
    public static readonly KEY_MAC_ENTER = 3;
    /**
     * Key code for pause key
     */
    public static readonly KEY_PAUSE = 19;
    /**
     * Key code for caps lock key
     */
    public static readonly KEY_CAPS_LOCK = 20;
    /**
     * Key code for space
     */
    public static readonly KEY_SPACE = 32;
    /**
     * Key code for print key
     */
    public static readonly KEY_PRINT_SCREEN = 44;
    /**
     * Key code for insert key (Also numeric keyboard insert).
     */
    public static readonly KEY_INSERT = 45;      // also NUM_INSERT
    /**
     * Key code for insert key (Also num lock on FF,Safari Mac).
     */
    public static readonly KEY_NUM_CENTER = 12;
    /**
     * Key code for left windows key.
     */
    public static readonly KEY_WIN_KEY = 224;
    /**
     * Key code for left windows key or meta.
     */
    public static readonly KEY_WIN_KEY_LEFT_META = 91;
    /**
     * Key code for right windows key.
     */
    public static readonly KEY_WIN_KEY_RIGHT = 92;
    /**
     * Key code for context menu key.
     */
    public static readonly KEY_CONTEXT_MENU = 93;
    /**
     * Key code for {@link KeyCodes#KEY_WIN_KEY_LEFT_META} that Firefox fires
     * for the meta key.
     */
    public static readonly KEY_MAC_FF_META = 224; // Firefox (Gecko) fires this for the meta key instead of 91
    /**
     * Key code for num lock.
     */
    public static readonly KEY_NUMLOCK = 144;
    /**
     * Key code for scroll lock.
     */
    public static readonly KEY_SCROLL_LOCK = 145;
    /**
     * Key code for first OS specific media key (like volume).
     */
    public static readonly KEY_FIRST_MEDIA_KEY = 166;
    /**
     * Key code for last OS specific media key (like volume).
     */
    public static readonly KEY_LAST_MEDIA_KEY = 183;
    /**
     * Key code for IME.
     */
    public static readonly KEY_WIN_IME = 229;
    /**
     * Key code for open square bracket, [.
     */
    public static readonly KEY_OPEN_BRACKET = 219;
    /**
     * Key code for close square bracket, ].
     */
    public static readonly KEY_CLOSE_BRACKET = 221;
    /**
     * Determines if a key code is an arrow key.
     */
    public static isArrowKey(code: int): boolean {
        switch (code) {
            case KeyCodes.KEY_DOWN:
            case KeyCodes.KEY_RIGHT:
            case KeyCodes.KEY_UP:
            case KeyCodes.KEY_LEFT:
                return true;
            default:
                return false;
        }
    }
    /**
     * Update arrow keys for left and right based on current locale.
     *
     * <p>Note: this method is used internally by built-in GWT widgets but
     *   could be renamed/refactored without notice.
     * <p>This method simplifies RTL handling in your code:
     * <code><pre>
     *  keyCode = KeyCodes.maybeSwapArrowKeysForRtl(event.getKeyCode(),
     *   LocaleInfo.getCurrentLocale().isRTL());
     * switch (keyCode) {
     *   case KeyCodes.KEY_LEFT:
     *     ... // start of the line, no special RTL handling
     *     break;
     *   case KeyCodes.KEY_RIGHT:
     *     ... // end of the line, no special RTL handling
     *     break;
     *   ...
     *   }
     * </pre></code>
     */
    public static maybeSwapArrowKeysForRtl(code: int, isRtl: boolean) {
        if (isRtl) {
            if (code === KeyCodes.KEY_RIGHT) {
                code = KeyCodes.KEY_LEFT;
            } else if (code === KeyCodes.KEY_LEFT) {
                code = KeyCodes.KEY_RIGHT;
            }
        }
        return code;
    }
    // This class should never be instantiated
    private KeyCodes() {
    }
}