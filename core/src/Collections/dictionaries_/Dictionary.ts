...
export class _Dictionary<TKey, TValue> extends DictionaryBase<TKey, TValue>
{
	// Retains the order...
	...
	protected _setValueInternal(key: TKey, value: TValue | undefined): boolean {
		...
		if (bucketEntry) {
			...
			if (!bucket) throw new Error(`"${<any>hash}" cannot be added to lookup table.`);
