...
export class CharacterSplitter {
    private isSurrogate(str: string, pos: int): boolean {
    ...
    private codePointAt(str: string, idx: int): int {
        if (idx === undefined) {
            idx = 0;
        }
    ...
    private shouldBreak(start: int, mid: int[], end: int) {
    ...
    private nextBreak(string: string, index: int): number {
        if (index === undefined) {
            index = 0;
        }
        if (index < 0) {
        ...
        const prev = this.getGraphemeBreakProperty(this.codePointAt(string, index));
        const mid: int[] = []
        for (var i = index + 1; i < string.length; i++) {
            // check for already processed low surrogates
            if (this.isSurrogate(string, i - 1)) {
                continue;
            }

            const next = this.getGraphemeBreakProperty(this.codePointAt(string, i));
            if (this.shouldBreak(prev, mid, next)) {
                return i;
            }
    ...
    private iterateGraphemes(str: string) {
        var index = 0;
        var res = {
            next: (function () {
                var value;
    ...
    private countGraphemes(str: string): number {
        var count = 0;
        var index = 0;
        var brk;
        while ((brk = this.nextBreak(str, index)) < str.length) {
    ...
    private getGraphemeBreakProperty(code: int): int {
