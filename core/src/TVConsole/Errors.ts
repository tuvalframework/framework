...
export class Errors {
    ...
    public constructor(tvc: TVC) {
        this.tvc = tvc;
        this.language = 'en';	//this.tvc.utilities.getBrowserLanguage();
        ...
        this.errors =
        {
            en:
                [
                    ...
                    "illegal_function_call:Illegal function call: %1 %2 %3",
        ...
        function getError(errorList, error) {
            if (errorList) {
                ...
                if (number >= 0) {
                    ...
                    if (error.parameter)
                        message = self.tvc.utilities.replaceStringInText(message, '%1', '' + error.parameter);
                    else if (error.parameters) {
                        for (var p = 0; p < error.parameters.length; p++)
                            message = self.tvc.utilities.replaceStringInText(message, '%' + (p + 1), '' + error.parameters[p]);
                    }
    ...
    public getErrorFromNumber(number: int) {
        var self = this;
