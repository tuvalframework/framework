...
// Fonts definitions...
const AmigaFonts: any = {};
const AmigaCharacters: any = {};
const GoogleFonts: any = {};
const BitmapFonts: any = {};
...
export class Fonts {
    ...
    public constructor(tvc: TVC) {
        ...
        this.miniCanvas.height = 16;
        this.miniContext = this.miniCanvas.getContext('2d')!;
        this.fontInformations = [];
        this.windowFonts = {};
        this.windowFontsInformations = {};

        this.setWindowFont("IBM Plex Mono", "google", 40, "normal", "normal", "normal", "", "application");
        ...
        if (reference) {
            this.tvc.loadingMax++;
            this.getFont(reference, this.firstFontHeight, undefined, undefined, undefined, '', this.tvc.currentContextName, function (response, font, extra) {
    ...
    private static EMPTYAMIGACHAR_KERN: int = 2;
    private static EMPTYAMIGACHAR_SPACE: int = 2;
    private static EMPTYAMIGACHAR_WIDTH: int = 4;

    private getFonts(googleList, amigaList, contextName) {
        ...
        for (var a = 0; a < amigaList.length; a++) {
            ...
            var fontInformation: any = this.fontInformations.find(function (element) { return element.name.toLowerCase() == name.toLowerCase(); });
            if (!fontInformation) {
                fontInformation =
                {
                    index: name.toLowerCase(),
    ...
    private setWindowFont(name, type, height, weight, italic, stretch, tags, contextName) {
        var fontInformation;
    ...
    private getFontInformation(reference, contextName) {
    ...
    private getAmigaFontDefinition(font, height?) {
        var result: any = {};
    ...
    private getFirstFont() {
        for (var reference in this.fontInformations) {
            if (this.fontInformations[reference].contextNames[this.tvc.currentContextName])
                return this.fontInformations[reference];
        }
        return null;
    };
    private getFont(reference, height, weight, italic, stretch, tags, contextName, callback?, extra?) {
        ...
        this.loadFont(fontInformation, weight, italic, stretch, function (response, font, extra) {
            if (response) {
                ...
                if (self.tvc.utilities.isTag(tags, ['setWindowFont']))
    ...
    public loadFont(fontInformation, weight, italic, stretch, callback, extra?) {
        ...
        if (fontInformation.type == 'google') {
            ...
            if (!fontInformation.fonts[fontStyleIndex]) {
                ...
                observer.load().then(function () {
                    ...
                    self.tvc.loadingCount++;
                    self.tvc.loadingError = 'cannot_load_font';
                    callback(false, null, extra);
                });
            }
            else {
    ...
    private getFont$(reference) {
        var result = '';
        var fontInformation = this.getFontInformation(reference, this.tvc.currentContextName);
        if (fontInformation && fontInformation.contextNames[this.tvc.currentContextName]) {
    ...
    private getBaseLine(font, height) {
        return font.baseLines[height];
    };
    private getTextLength(text, font, height) {
    ...
    public drawAmigaText(context, scale, x, y, text, font, height, textAlign, textBaseLine, direction, fillStyle, alpha, fontWidth) {
        ...
        for (var c = 0; c < text.length; c++) {
            ...
            context.drawImage(result.canvas, 0, 0, result.canvas.width, result.canvas.height, x * scale + xx * scaleDisplay, y * scale, result.canvas.width * scaleDisplay, height * scaleDisplay);
            if (!fontWidth)
                xx += result.charDefinition.kern + result.charDefinition.space;
            else
                xx += fontWidth;
        }
    };
    private getAmigaCharacter(font, fontDefinition, c, height, fillStyle) {
        var charDefinition;
        if (c >= fontDefinition.heightDefinition.loChar && c <= fontDefinition.heightDefinition.hiChar)
        ...
        if (charDefinition.bitmap) {
            ...
            for (var y = 0; y < font.baseHeight; y++) {
                ...
                var mask = 0x80;
                for (let x = 0; x < charDefinition.width; x++) {
                    if ((byte & mask) != 0) {
            ...
            // Try to find the default width of char not defined

        }
        return { canvas: canvas, charDefinition: charDefinition };
    };

    private getTextMetrics(fontFamily, fontSize, fontWeight) {
        var padding;
        var context;
        var canvas;
        var chars =
    ...
    private bitmapFont(fontRef, charWidth, charHeight, imageRef, scaleX, scaleY) {
    ...
    private bitmapText(text, x, y, fontRef, fontStyle) {
        ...
        var image = this.tvc.fonts.banks.getImage('images', bFont.image, this.tvc.currentContextName, 1);
        if (image.canvas) {
            var canvas = document.createElement('canvas');
            ...
            this.tvc.fonts.banks.insertImage('images', "bitmap_text_tvc", undefined, undefined, this.tvc.currentContextName, undefined, canvas);
            this.tvc.currentScreen.pasteImage('images', "bitmap_text_tvc", x, y, undefined, undefined, -((undefined as any) * this.tvc.degreeRadian));
        }
    };
}
