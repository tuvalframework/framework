...
class HebrewValue {
    public /* internal */  token: HebrewToken;
    public /* internal */  value: int;
    public /* internal */ constructor(token: HebrewToken, value: int) {
        this.token = token;
        this.value = value;
    }
}

export enum HebrewNumberParsingState {
    InvalidHebrewNumber,
    NotHebrewDigit,
...
export class HebrewNumber {
    ...
    public /* internal */ static ToString(Number: int): string {
        let cTens: char = '\x00'.charCodeAt(0);
        let cUnits: char;               // tens and units chars
        let Hundreds: int, Tens: int;              // hundreds and tens values
        const szHebrew: StringBuilder = new StringBuilder();
        ...
        if (Hundreds > 0) {
            ...
            for (let i: int = 0; i < (Hundreds / 4); i++) {
                szHebrew.Append('\x05ea');
            }

            const remains: int = Hundreds % 4;
            if (remains > 0) {
                szHebrew.AppendChar(('\x05e6'.charCodeAt(0) + remains));
            }
        }
        ...
        switch (Tens) {
            case (0):
                cTens = '\x00'.charCodeAt(0);
                break;
            case (1):
                cTens = '\x05d9'.charCodeAt(0);          // Hebrew Letter Yod
                break;
            case (2):
                cTens = '\x05db'.charCodeAt(0);          // Hebrew Letter Kaf
                break;
            case (3):
                cTens = '\x05dc'.charCodeAt(0);          // Hebrew Letter Lamed
                break;
            case (4):
                cTens = '\x05de'.charCodeAt(0);          // Hebrew Letter Mem
                break;
            case (5):
                cTens = '\x05e0'.charCodeAt(0);          // Hebrew Letter Nun
                break;
            case (6):
                cTens = '\x05e1'.charCodeAt(0);          // Hebrew Letter Samekh
                break;
            case (7):
                cTens = '\x05e2'.charCodeAt(0);          // Hebrew Letter Ayin
                break;
            case (8):
                cTens = '\x05e4'.charCodeAt(0);          // Hebrew Letter Pe
                break;
            case (9):
                cTens = '\x05e6'.charCodeAt(0);          // Hebrew Letter Tsadi
                break;
        }
        ...
        //  Get the Units.
        //
        cUnits = Convert.ToChar(Number > 0 ? ('\x05d0'.charCodeAt(0) + Number - 1) : 0);

        if ((cUnits === '\x05d4'.charCodeAt(0)) &&            // Hebrew Letter He  (5)
            (cTens === '\x05d9'.charCodeAt(0))) {              // Hebrew Letter Yod (10)
            cUnits = '\x05d5'.charCodeAt(0);                 // Hebrew Letter Vav (6)
            cTens = '\x05d8'.charCodeAt(0);                 // Hebrew Letter Tet (9)
        }

        if ((cUnits === '\x05d5'.charCodeAt(0)) &&            // Hebrew Letter Vav (6)
            (cTens === '\x05d9'.charCodeAt(0))) {               // Hebrew Letter Yod (10)
            cUnits = '\x05d6'.charCodeAt(0);                 // Hebrew Letter Zayin (7)
            cTens = '\x05d8'.charCodeAt(0);                 // Hebrew Letter Tet (9)
        }
        ...
        if (cTens !== '\x00'.charCodeAt(0)) {
            szHebrew.AppendChar(cTens);
        }

        if (cUnits !== '\x00'.charCodeAt(0)) {
            szHebrew.AppendChar(cUnits);
        }
    ...
    //
    private static HebrewValues: HebrewValue[] = [
    ...
    private static readonly minHebrewNumberCh: int = 0x05d0;
    private static maxHebrewNumberCh: char = Convert.ToChar(HebrewNumber.minHebrewNumberCh + HebrewValue.length - 1);
    ...
    //
    private static readonly NumberPasingState: HS[][] = [
                       // 400            300/200         100             90~10           8~1      6,       7,       9,          '           "
                    /* 0 */[HS.S400, HS.X00, HS.X00, HS.X0, HS.X, HS.X, HS.X, HS.S9, HS._err, HS._err],
                    /* 1: S400 */[HS.S400_400, HS.S400_X00, HS.S400_X00, HS.S400_X0, HS._err, HS._err, HS._err, HS.X00_S9, HS.END, HS.S400_DQ],
                    /* 2: S400_400 */[HS._err, HS._err, HS.S400_400_100, HS.S400_X0, HS._err, HS._err, HS._err, HS.X00_S9, HS._err, HS.S400_400_DQ],
                    /* 3: S400_X00 */[HS._err, HS._err, HS._err, HS.S400_X00_X0, HS._err, HS._err, HS._err, HS.X00_S9, HS._err, HS.X00_DQ],
                    /* 4: S400_X0 */[HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS.X0_DQ],
                    /* 5: X00_DQ */[HS._err, HS._err, HS._err, HS.END, HS.END, HS.END, HS.END, HS.END, HS._err, HS._err],
                    /* 6: S400_X00_X0 */[HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS.X0_DQ],
                    /* 7: X0_DQ */[HS._err, HS._err, HS._err, HS._err, HS.END, HS.END, HS.END, HS.END, HS._err, HS._err],
                    /* 8: X */[HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS.END, HS._err],
                    /* 9: X0 */[HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS.END, HS.X0_DQ],
                    /* 10: X00 */[HS._err, HS._err, HS._err, HS.S400_X0, HS._err, HS._err, HS._err, HS.X00_S9, HS.END, HS.X00_DQ],
                    /* 11: S400_DQ */[HS.END, HS.END, HS.END, HS.END, HS.END, HS.END, HS.END, HS.END, HS._err, HS._err],
                    /* 12: S400_400_DQ*/[HS._err, HS._err, HS.END, HS.END, HS.END, HS.END, HS.END, HS.END, HS._err, HS._err],
                    /* 13: S400_400_100*[HS._err, HS._err, HS._err, HS.S400_X00_X0, HS._err, HS._err, HS._err, HS.X00_S9, HS._err, HS.X00_DQ },
                    /* 14: S9 */        [HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS.END, HS.S9_DQ],
                    /* 15: X00_S9 */[HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS._err, HS.S9_DQ],
                    /* 16: S9_DQ */[HS._err, HS._err, HS._err, HS._err, HS._err, HS.END, HS.END, HS._err, HS._err, HS._err]
    ];
    ...
    public /* internal */ static ParseByChar(ch: char, context: HebrewNumberParsingContext): HebrewNumberParsingState {
        let token: HebrewToken;
        if (ch === '\''.charCodeAt(0)) {
            token = HebrewToken.SingleQuote;
        } else if (ch === '\"'.charCodeAt(0)) {
            token = HebrewToken.DoubleQuote;
        } else {
            const index: int = Convert.ToInt32(ch) - HebrewNumber.minHebrewNumberCh;
            if (index >= 0 && index < HebrewValue.length) {
                token = HebrewValue[index].token;
                if (token === HebrewToken.Invalid) {
                    return (HebrewNumberParsingState.NotHebrewDigit);
                }
                context.result += HebrewValue[index].value;
            } else {
                // Not in valid Hebrew digit range.
                return (HebrewNumberParsingState.NotHebrewDigit);
            }
        }
        context.state = HebrewNumber.NumberPasingState[context.state][token];
    ...
    public /* internal */ static IsDigit(ch: char): boolean {
        if (ch >= HebrewNumber.minHebrewNumberCh && ch <= HebrewNumber.maxHebrewNumberCh) {
            return (HebrewValue[ch - HebrewNumber.minHebrewNumberCh].value >= 0);
        }
