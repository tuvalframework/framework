...
const subKeys: string = 'TICKER|TICKER_END|CANVAS|CANVAS_END|noLoop';
const keywords = new RegExp("^(" + subKeys + ")$", 'i');
...
export class Parser {
    public static InstructionInfo: any = {};
    ...
    // Expect the next token to match the any Instruction.
    // If not, an exception will be thrown.

    expectInstruction(instruction: string) {
        const token = this.nextToken();
        if (token.type !== Token.Instruction || token.value !== instruction) {
            this.throwUnexpectedToken(token);
        }
    }
    ...
    // To için eklendi.
    match1(value, type) {
        return this.lookahead.type === type && this.lookahead.value === value;
    }
    ...
    matchInstructor(value) {
        return this.lookahead.type === Token.Instruction && this.lookahead.value === value;
    }
    ...
    parseExpression(): Node.Expression | Node.SequenceExpression {
        ...
        if (this.match(',') || this.match1('To', Token.Identifier)) {
            const expressions: Node.Expression[] = [];
            expressions.push(expr);
            while (this.lookahead.type !== Token.EOF) {
                if (!this.match(',') && !this.match1('To', Token.Identifier)) {
                ...
                if (this.match1('To', Token.Identifier)) {
                    this.nextToken();
                    expressions.push(this.parseToExpression());
                } else {
                    this.nextToken();
    ...
    parseToExpression(): Node.Expression | Node.ToExpression {
        const expressions: Node.Expression[] = [];
        ...
        expr = this.finalize(this.startNode(startToken), new Node.ToExpression(expressions));
        return expr;
    }
    ...
    parseInstructionStatement(instruction: string): Node.InstructionStatement {
        const node = this.createNode();
        let body;
        let paramsExp;

        this.expectInstruction(instruction);
        const info = Parser.InstructionInfo[instruction];
        if (info) {
            if (info.argumentCount > 0) {
                paramsExp = this.parseExpression(); /* this.parseInstructionFormalParameters(); */
            }
        } else {
            paramsExp = this.parseExpression();
        }
        if (this.scanner.isBlockedInstruction(instruction)) {
            body = this.parseInstructionBlock(instruction);
            this.expectInstruction(instruction + ' End');
        }
        if (this.match(';') || this.match(':')) {
            this.nextToken();
        }
        ...
        return this.finalize(node, new Node.InstructionStatement(instruction, body, paramsExp));
    }

    parseInstructionFormalParameters(): any[] {
        const options: any = {
            simple: true,
            params: [],
        };

        while (this.lookahead.type !== Token.EOF && !this.hasLineTerminator) {
            if (this.lookahead.type === Token.Identifier) {
                const name = this.parseIdentifierName();
                options.params.push(name);
            } else if (this.lookahead.type === Token.NumericLiteral) {
                const statement = this.parseExpressionStatement();
                options.params.push(statement);
            }
            if (this.match(',')) {
                this.expect(',');
            }
    ...
    parseInstructionBlock(instruction: string): Node.BlockStatement {
        const node = this.createNode();

        //this.expect('{');
        const block: Node.StatementListItem[] = [];
        while (true) {
            if (this.matchInstructor(instruction + ' End')) {
                break;
            }
            block.push(this.parseStatementListItem());
        }
        //this.expectInstruction('TICKER_END');
    ...
    parseStatement(): Node.Statement {
        ...
        switch (this.lookahead.type) {
            ...
            case Token.Instruction:
                statement = this.parseInstructionStatement((this.lookahead as any).value);
                break;
            case Token.Keyword:
                switch (this.lookahead.value) {
                    case 'break':
