...
export class ArrayExpression {
    ...
    public toString(info?: any): string {
        if (info && info['ArrayExpression']) {
            return info['ArrayExpression'](this);
        } else {
            let elements: string = '';
            for (let i = 0; i < this.elements.length; i++) {
                ...
                elements += this.elements[i]?.toString(info);
            }
            return `[${elements}]`;
        }
...
export class AssignmentExpression {
    ...
    public toString(info?: any): string {
        if (info && info['AssignmentExpression']) {
            return info['AssignmentExpression'](this);
        } else {

            return `${this.left.toString(info)} ${this.operator} ${this.right.toString(info)}`;
...
export class BinaryExpression {
    ...
    public toString(info?: any): string {
        if (info && info['BinaryExpression']) {
            return info['BinaryExpression'](this);
        } else {
            return `${this.left.toString(info)} ${this.operator} ${this.right.toString(info)}`;
        }
    }
...
export class BlockStatement {
    ...
    public toString(info?: any): string {
        if (info && info['BlockStatement']) {
            return info['BlockStatement'](this);
        } else {
            let body: string = '';
        ...
        return body; */
        /* InstructionStatement.BlockIndex++;
        let body: string = '';
        const self = this;
        function loop(loopi) {
             body += `this.blocks[${InstructionStatement.BlockIndex}] = function () {`;

            let bi = InstructionStatement.BlockIndex;
            for (let i = loopi; i < self.body.length; i++) {
                body += `${self.body[i].toString(info)};`;
                if (bi !== InstructionStatement.BlockIndex) {
                    body += '};\n`';
                    loop(i + 1);
                }
            }
...
export class CallExpression {
    ...
    public toString(info: any): string {
        if (info && info['CallExpression']) {
            return info['CallExpression'](this);
        } else {
            let args: string = '';
            for (let i = 0; i < this.arguments.length; i++) {
                ...
                args += this.arguments[i].toString(info);
            }
...
export class ComputedMemberExpression {
    ...
    public toString(info?: any): string {
        if (info && info['ComputedMemberExpression']) {
            return info['ComputedMemberExpression'](this);
        } else {
            return `${this.object.toString(info)}[${this.property.toString(info)}]`;
        }
    }
}
...
export class ContinueStatement {
    ...
    public toString(info?: any): string {
        if (info && info['ContinueStatement']) {
            return info['ContinueStatement'](this);
        } else {
            if (this.label == null) {
                return 'constinue;';
            } else {
                return `continue ${this.label.toString(info)};`
            }
        }
    }
}
...
export class ExpressionStatement {
    ...
    public toString(info?: any): string {
        if (info && info['ExpressionStatement']) {
            return info['ExpressionStatement'](this);
        } else {
            return this.expression.toString(info);
        }
    }
}
...
export class ForStatement {
    ...
    public toString(info?: any): string {
        if (info && info['ForStatement']) {
            return info['ForStatement'](this);
        } else {
            let body = '';
            if (this.body instanceof BlockStatement) {
                for (let i = 0; i < this.body.body.length; i++) {
                    body += '\t\t\t\t\t' + this.body.body[i].toString(info) + ';\n';
                }
                return `for(${(this as any).init?.toString(info, true)};${this.test?.toString(info)};${this.update?.toString(info)}) {
                    ${body}
                }`;
            } else {
                return `\t\t\t\t\tfor(${(this as any).init?.toString(info, true)};${this.test?.toString(info)};${this.update?.toString(info)}) {
                        ${this.body.toString(info)}
                   }`;
            }
        }
    }
...
export class FunctionDeclaration {
    ...
    public toString(info?: any): string {
        if (info && info['FunctionDeclaration']) {
            return info['FunctionDeclaration'](this);
        } else {
            let params: string = '';
...
export class FunctionExpression {
    ...
    public toString(info?: any): string {
        if (info && info['FunctionExpression']) {
            return info['FunctionExpression'](this);
        } else {
            let params = '';
            for (let i = 0; i < this.params.length; i++) {
                if (i > 0) {
                ...
                params += this.params[i].toString(info);
            }
...
export class IfStatement {
    ...
    public toString(info?: any): string {
        if (info && info['IfStatement']) {
            return info['IfStatement'](this);
        } else {
            const test = this.test.toString(info);
            const str1 = `if(${test}) {
               ${this.consequent.toString(info)}
           }`;
            let str2 = '';
            if (this.alternate != null) {
                str2 = ` else {
                ${this.alternate.toString(info)}
            }`;
            }
            return str1 + str2;
        }
...
export class LabeledStatement {
    ...
    public toString(info?: any): string {
        if (info && info['LabeledStatement']) {
            return info['LabeledStatement'](this);
        } else {
            let body: string = `${this.label.toString(info)}:\n`;
            body += this.body.toString(info);
            return body;
        }
    }
...
export class ObjectExpression {
    ...
    public toString(info?: any): string {
        ...
        for (let i = 0; i < this.properties.length; i++) {
            ...
            properties += this.properties[i].toString(info);
        }
        return `{ ${properties} }`;
    }
}
...
export class Property {
    ...
    public toString(info: string): string {
        return `${this.key.toString(info)} : ${this.value?.toString(info)}`;
    }
}
...
export class ToExpression {
    ...
    public toString(info?: any): string {
        if (info && info['ToExpression']) {
            return info['ToExpression'](this);
        } else {
            ...
            for (let i = 0; i < this.expressions.length; i++) {
                ...
                expressions += this.expressions[i].toString(info);
            }
            return expressions;
        }
    }
...
export class SequenceExpression {
    ...
    public toString(info?: any): string {
        if (info && info['SequenceExpression']) {
            return info['SequenceExpression'](this);
        } else {
            let expressions: string = '';
            for (let i = 0; i < this.expressions.length; i++) {
                if (i > 0) {
...
export class StaticMemberExpression {
    ...
    public toString(info?: any): string {
        if (info && info['StaticMemberExpression']) {
            return info['StaticMemberExpression'](this);
        } else {
...
export class UnaryExpression {
    ...
    public toString(info?: any): string {
        if (info && info['UnaryExpression']) {
            return info['UnaryExpression'](this);
        } else {
            if (this.prefix) {
                return `${this.operator}${this.argument.toString(info)}`;
            } else {
                return `${this.argument.toString(info)}${this.operator}`;
            }
        }
    }
}
...
export class UpdateExpression {
    ...
    public toString(info?: any): string {
        if (info && info['UpdateExpression']) {
            return info['UpdateExpression'](this);
        } else {
            if (this.prefix) {
                return `${this.operator}${this.argument.toString(info)};`;
            } else {
...
export class VariableDeclaration {
    ...
    public toString(info?, inline: boolean = false): string {
        if (info && info['VariableDeclaration']) {
            return info['VariableDeclaration'](this);
        } else {
            let declarations = '';
            for (let i = 0; i < this.declarations.length; i++) {
                ...
                declarations += this.declarations[i].toString(info);
            }
            if (inline) {
                return `${this.kind} ${declarations}`;
            } else {
                return `${this.kind} ${declarations};\n`;
            }
        }
...
export class VariableDeclarator {
    ...
    public toString(info?): string {
        if (info && info['VariableDeclarator']) {
            return info['VariableDeclarator'](this);
        } else {
            if (this.init == null) {
                return `${this.id.toString()}`;
            } else {
                return `${this.id.toString()} = ${this.init}`;
            }
        }
    }
}
...
export class WhileStatement {
    ...
    public toString(info?: any): string {
        if (info && info['WhileStatement']) {
            return info['WhileStatement'](this);
        } else {
            const test = this.test.toString(info);
            return `while(${test}) {
               ${this.body.toString(info)}
...
export class InstructionStatement {
    public static BlockIndex: number = 0;
    readonly type: string;
    readonly name: string;
    readonly body: Statement;
    readonly params: SequenceExpression;
    static readonly instructions: any = {};
    constructor(name: string, body: Statement, params: SequenceExpression) {
        this.type = Syntax.InstructionStatement;
        this.name = name;
        this.body = body;
        this.params = params;
    }
    public static AddInstruction(intructionClass: InstructionBase): void {
        const name = intructionClass.GetName();
        if (name == null) {
            throw 'Instruction Name required.';
        }
        InstructionStatement.instructions[name] = intructionClass;
        const subIns = intructionClass.GetSubInstructions();
        for (let i = 0; i < subIns.length; i++) {
            const sub = subIns[i];
            InstructionStatement.instructions[sub.GetName()] = sub;
        }
    }
    public toString(info?): string {
        const inst = InstructionStatement.instructions[this.name];
        if (inst) {
            return inst.toString(this, info);
        }
        return '';
    }
}
