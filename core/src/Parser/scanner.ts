...
export class Scanner {
    public static subKeys: string = 'TICKER|TICKER_END|CANVAS|CANVAS_END|noLoop';
    ...
    private isInstruction(id: string): boolean {
        const keywords = new RegExp("^(" + Scanner.subKeys + ")$", 'i');
        let r = id.match(keywords);
        if (r && r[0]) {
            return true;
        }
    ...
    public isBlockedInstruction(id: string): boolean {
        const keywords = new RegExp("^(" + Scanner.subKeys + ")$", 'i');
        id = id + ' End';
        let r = id.match(keywords);
        if (r && r[0]) {
    ...
    private getInstruction(start: number): string {
        let i = this.index;
        while (true) {
            const ch = this.source.charCodeAt(i);
            const temp = this.source.slice(start, i);
            const char = String.fromCharCode(ch);
            if (Character.isIdentifierPart(ch) || ch === 32) {
                i++;
            } else {
                break;
            }
        ...
        do {
            const temp = this.source.slice(start, i);
            if (this.isInstruction(temp) && !Character.isIdentifierPart(this.source.slice(i, i + 1).charCodeAt(0))) {// bir sonraki boşluk yada farklı birşey olmalı
                this.index = i;
                return temp;
            }
            i--;
    ...
    private getIdentifier(): string {
        ...
        const instruction = this.getInstruction(start);
        if (instruction) {
            return instruction;
        }
        ...
        while (!this.eof()) {
            ...
            /* if (ch === 32) {
                const temp = this.source.slice(start, this.index);
                let i = this.index;
                if (this.isInstruction(temp)) {
                    break;
                } else {
                    while(true) {
                        const temp1 = this.source.slice(start, i);
                        if (this.isInstruction(temp1)) {
                            this.index = i;
                            return temp1;
                        } else {
                            i++;
