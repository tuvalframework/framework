...
function findMatchedRoutes(url, routes: any[] = []) {
    return routes
        .map(route => {
...
function manageHooks(handler, hooks, params?, arg?) {
...
export class Router {
    public static PARAMETER_REGEXP = /([:*])(\w+)/g;
    public static WILDCARD_REGEXP = /\*/g;
    public static REPLACE_VARIABLE_REGEXP = '([^\/]+)';
    public static REPLACE_WILDCARD = '(?:.*)';
    public static FOLLOWED_BY_SLASH_REGEXP = '(?:\/$|$)';
    public static MATCH_REGEXP_FLAGS = '';
    private root: any;
    private _routes: any[];
    ...
    private _lastRouteResolved: any;
    private _notFoundHandler: any;
    private _defaultHandler: any;
    private _usePushState: boolean;
    private _genericHooks: any;
    private _historyAPIUpdateMethod: string;
    private _listeningInterval: any = undefined as any;
    public constructor(r, useHash?, hash?) {
        this.root = null;
        this._routes = [];
    ...
    public notFound(handler, hooks) {
        this._notFoundHandler = { handler, hooks: hooks };
        return this;
    }
    public resolve(current?) {
        var handler, m;
    ...
    public destroy() {
        ...
        this._lastRouteResolved = null;
        this._genericHooks = null;
        clearTimeout(this._listeningInterval as any);
    ...
    public updatePageLinks() {
        ...
        this._findLinks().forEach(link => {
            if (!(link as any).hasListenerAttached) {
                (link as any).addEventListener('click', function (e) {
    ...
    public historyAPIUpdateMethod(value) {
        if (typeof value === 'undefined') return this._historyAPIUpdateMethod;
        this._historyAPIUpdateMethod = value;
        return value;
    }
    public disableIfAPINotAvailable() {
    ...
    public getLinkPath(link) {
        return link.getAttribute('href');
    }
    public hooks(hooks) {
        this._genericHooks = hooks;
    }
    private _add(route, handler: any = null, hooks = null) {
    ...
    private _listen() {
        if (this._usePushState) {
            ...
            check = () => {
                ...
                this._listeningInterval = setTimeout(check, 200) as any;
            };
            check();
        }
    }
...
(Router.prototype as any).helpers =  {
        match,
        root,
        clean,
        getOnlyURL
