...
export const VirtualScroller = React.memo(React.forwardRef((props, ref) => {
    ...
    const scrollInView = (index, to, behavior = 'auto') => {
        if (to) {
            ...
            if (isToStart) {
                ...
                if (both) {
                    if ((viewport as any).last.rows - first.rows <= index[0] + 1) {
                        scrollToItem(viewport.first.cols * itemSize, (viewport.first.rows + 1) * itemSize);
                    }
                    else if ((viewport as any).last.cols - first.cols <= index[1] + 1) {
                        scrollToItem((viewport.first.cols + 1) * itemSize, viewport.first.rows * itemSize);
                    }
                }
                else {
    ...
    const calculateNumItems = () => {
        ...
        const numItemsInViewport: any = both ?
    ...
    const calculateOptions = () => {
        ...
        const calculateLast: any = (_first, _num, _numT, _isCols) => getLast(_first + _num + ((_first < _numT ? 2 : 3) * _numT), _isCols);
        const last = both ?
    ...
    const onScrollPositionChange = (event) => {
        ...
        const calculateLast: any = (_currentIndex, _first, _last, _num, _numT, _isCols) => {
    ...
    if (props.disabled) {
        ...
        const otherProps = ObjectUtils.findDiffKeys(props, (VirtualScroller as any).defaultProps);
...
(VirtualScroller as any).defaultProps = {
