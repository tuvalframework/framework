...
export const DataTable : any= React.forwardRef((props, ref) => {
    ...
    const _restoreState = (restoredState: any = {}) => {
    ...
    const cloneFilters = (filters) => {
        ...
        if (filters) {
            Object.entries(filters).forEach(([prop, value]) => {
                cloned[prop] = (value as any).operator ? { operator: (value as any).operator, constraints: (value as any).constraints.map(constraint => { return { ...constraint } }) } : { ...(value as any) };
            });
        }
        else {
