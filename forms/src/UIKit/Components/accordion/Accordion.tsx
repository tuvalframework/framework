...
export const Accordion = React.forwardRef((props, ref) => {
    ...
    const createTab = (tab, index) => {
        if (shouldUseTab(tab)) {
            ...
            const selected = isSelected(index);
            const otherProps = ObjectUtils.findDiffKeys(tab.props, (AccordionTab as any).defaultProps);
    ...
    const otherProps = ObjectUtils.findDiffKeys(props, (Accordion as any).defaultProps);
...
(AccordionTab as any).defaultProps = {
    __TYPE: 'AccordionTab',
    header: null,
    disabled: false,
    style: null,
