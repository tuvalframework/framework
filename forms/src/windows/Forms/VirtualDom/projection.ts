...
export let initPropertiesAndChildren = (domNode: Node, vnode: VNode, projectionOptions: ProjectionOptions) => {
  ...
  if (vnode.properties && vnode.properties.afterCreate) {
    vnode.properties.afterCreate.apply(
      ...
      [domNode as Element, projectionOptions, vnode.vnodeSelector, vnode.properties, vnode.children as any]
    );
  }
};
...
export let createDom = (
  ...
  let doc: any = parentNode.ownerDocument;
  if (vnodeSelector === '') {
    domNode = vnode.domNode = doc.createTextNode(vnode.text!);
    if (insertBefore !== undefined) {
      parentNode.insertBefore(<any>domNode, insertBefore);
    } else {
      parentNode.appendChild(<any>domNode);
    }
  } else {
    for (let i = 0; i <= vnodeSelector.length; ++i) {
      ...
      if (i === vnodeSelector.length || c === '.' || c === '#') {
        ...
        if (type === '.') {
          ...
          if (insertBefore !== undefined) {
            parentNode.insertBefore(<any>domNode, insertBefore);
          } else if ((<any>domNode).parentNode !== parentNode) {
            parentNode.appendChild(<any>domNode);
          }
        }
        start = i + 1;
...
updateDom = (previous, vnode, projectionOptions) => {
  ...
  if (vnode.vnodeSelector === '') {
    if (vnode.text !== previous.text) {
      let newTextNode = (<any>domNode).ownerDocument.createTextNode(vnode.text!);
    ...
    if (vnode.properties && vnode.properties.afterUpdate) {
      vnode.properties.afterUpdate.apply(
        vnode.properties.bind || vnode.properties,
        [<Element>domNode, projectionOptions, vnode.vnodeSelector, vnode.properties, vnode.children as any]
      );
    }
  }
