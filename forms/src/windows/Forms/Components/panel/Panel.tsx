...
import { classNames, foreach, List } from '@tuval/core';
...
export class PanelComponent extends React.Component {
        ...
        return (
            <CSSTransition nodeRef={this.contentRef} classNames="p-toggleable-content" timeout={{ enter: 1000, exit: 450 }} in={!collapsed} unmountOnExit options={this.props.transitionOptions}>
                <div ref={this.contentRef} className="p-toggleable-content" aria-hidden={collapsed} role="region" id={id} aria-labelledby={this.state.id + '_header'}>
                    <div className="p-panel-content" style={this.props.contentStyle}>
                        {this.props.children}
                    </div>
                </div>
            </CSSTransition>
    ...
    render() {
        const className = classNames('p-panel p-component', { 'p-panel-toggleable': this.props.toggleable } as any, this.props.className);
