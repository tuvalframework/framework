import React, { Component, createElement, createRef } from '../../../../preact/compat';
...
const css = require('./ContextMenu_.css');
DomHandler.addCssToDocument(css);

/**
 * 03.08.2021 OnBeforeShow eklendi.
 */

export class ContextMenuSubComponent extends React.Component {
    ...
    renderSubmenu(item) {
        if (item.items) {
            return (
                <ContextMenuSubComponent model={item.items} resetMenu={item !== this.state.activeItem} onLeafClick={this.props.onLeafClick} />
            );
        }
    ...
    renderMenuitem(item, index) {
        ...
        const className = classNames('p-menuitem', { 'p-menuitem-active': active } as any, item.className);
        const linkClassName = classNames('p-menuitem-link', { 'p-disabled': item.disabled } as any);
    ...
    render() {
        const className = classNames({ 'p-submenu-list': !this.props.root } as any);
...
export class ContextMenuComponent extends React.Component {
    ...
    show(event) {
        ...
        if (this.props.onBeforeShow) {
            this.props.onBeforeShow(this.props.model);
        }
        ...
        return (
            <CSSTransition nodeRef={this.menuRef} classNames="p-contextmenu" in={this.state.visible} timeout={{ enter: 250, exit: 0 }} options={this.props.transitionOptions}
                ...
                <div ref={this.menuRef} id={this.props.id} className={className} style={this.props.style} onClick={this.onMenuClick} onMouseEnter={this.onMenuMouseEnter}>
                    <ContextMenuSubComponent model={this.props.model} root resetMenu={this.state.resetMenu} onLeafClick={this.onLeafClick} />
                </div>
            </CSSTransition>
        );
    }
    ...
    render() {
        ...
        return React.createPortal(element, this.props.appendTo || document.body)
