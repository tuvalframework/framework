...
export class DomHandler {
    ...
    static generateZIndex() {
        this.zIndex = this.zIndex || Teact.zIndex;
        return ++this.zIndex;
    }

    static revertZIndex() {
        this.zIndex = Teact.zIndex < this.zIndex ? --this.zIndex : Teact.zIndex;
    }
    ...
    static removeCssToDocument(id: string) {
        const styles: HTMLCollection = document.getElementsByTagName('style');
        for (let i = 0; i < styles.length; i++) {
            if (styles[i].getAttribute('id') === id) {
