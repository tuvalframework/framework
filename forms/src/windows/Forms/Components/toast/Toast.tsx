import { DomHandler } from "../DomHandler";
import { classNames } from '@tuval/core';
import { Teact } from "../Teact";

import React from '../../../../preact/compat';
import { ZIndexUtils } from "../utils/ZIndexUtils";
import { TransitionGroup } from '../csstransition/TransitionGroup';
import { CSSTransition } from '../csstransition/CSSTransition';
import { ToastMessage } from './ToastMessage';

const css = require('./Toast.css');
DomHandler.addCssToDocument(css);

let messageIdx = 0;

export class Toast extends React.Component {

    static defaultProps = {
        id: null,
        className: null,
        style: null,
        baseZIndex: 0,
        position: 'top-right',
        transitionOptions: null,
        onClick: null,
        onRemove: null,
        onShow: null,
        onHide: null
    }
    container: any;

    constructor(props) {
        super( props);

        this.state = {
            messages: []
        };

        this.onClose = this.onClose.bind(this);
        this.onEntered = this.onEntered.bind(this);
        this.onExited = this.onExited.bind(this);
    }

    show(value) {
        if (value) {
            let newMessages;

            if (Array.isArray(value)) {
                for (let i = 0; i < value.length; i++) {
                    value[i].id = messageIdx++;
                    newMessages = [...this.state.messages, ...value];
                }
            }
            else {
                value.id = messageIdx++;
                newMessages = this.state.messages ? [...this.state.messages, value] : [value];
            }

            this.state.messages.length === 0 && ZIndexUtils.set('toast', this.container, this.props.baseZIndex);

            this.setState({
                messages: newMessages
            });
        }
    }

    clear() {
        ZIndexUtils.clear(this.container);

        this.setState({
            messages: []
        });
    }

    onClose(message) {
        this.setState({
            showMessage: false
        });

        let newMessages = this.state.messages.filter(msg => msg.id !== message.id);
        this.setState({
            messages: newMessages
        });

        if (this.props.onRemove) {
            this.props.onRemove(message);
        }


    }

    onEntered() {
        this.props.onShow && this.props.onShow();
    }

    onExited() {
        this.state.messages.length === 0 && ZIndexUtils.clear(this.container);

        this.props.onHide && this.props.onHide();

    }

    componentWillUnmount() {
        ZIndexUtils.clear(this.container);
    }

    /*     private getMessage(message): any {
            const messageRef = React.createRef();
            return (
                <CSSTransition nodeRef={messageRef} in={true} key={message.id} classNames="p-toast-message" unmountOnExit timeout={{ enter: 300, exit: 300 }} onEntered={this.onEntered} onExited={this.onExited} options={this.props.transitionOptions}>
                    <ToastMessage ref={messageRef} message={message} onClick={this.props.onClick} onClose={this.onClose} />
                </CSSTransition>
            )
        }
        private getMessages() {
            const vNodes: any[] = [];
            for (let i = 0; i < this.state.messages.length; i++) {
                const m = this.getMessage(this.state.messages[i]);
                vNodes.push(m);
            }
            return vNodes;

        } */
    render() {
        let className = classNames('p-toast p-component p-toast-' + this.props.position, this.props.className);

        return (
            <div ref={(el) => { this.container = el; }} id={this.props.id} className={className} style={this.props.style}>
                <TransitionGroup>
                    {
                        this.state.messages.map((message) => {
                            const messageRef = React.createRef();

                            return (
                                <CSSTransition nodeRef={messageRef} key={message.id} classNames="p-toast-message" unmountOnExit timeout={{ enter: 300, exit: 300 }} onEntered={this.onEntered} onExited={this.onExited} options={this.props.transitionOptions}>
                                    <ToastMessage ref={messageRef} message={message} onClick={this.props.onClick} onClose={this.onClose} />
                                </CSSTransition>
                            )
                        })
                    }
                </TransitionGroup>
            </div>
        );
    }
}