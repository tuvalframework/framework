...
const css = require('./FileUpload.css');
...
export class FileUploadComponent extends React.Component {
    ...
    renderFile(file, index) {
        ...
        let removeButton = <div><ButtonComponent type="button" icon="pi pi-times" onClick={(e) => this.remove(e, index)} /></div>
        let content = (
            <Fragment>
                {preview}
                {fileName}
    ...
    renderAdvanced() {
        ...
        if (!this.props.auto) {
            ...
            uploadButton = <ButtonComponent type="button" label={uploadLabel} icon={uploadOptions.icon || 'pi pi-upload'} onClick={this.upload} disabled={this.uploadDisabled()} style={uploadOptions.style} className={uploadOptions.className} />;
            cancelButton = <ButtonComponent type="button" label={cancelLabel} icon={cancelOptions.icon || 'pi pi-times'} onClick={this.clear} disabled={this.cancelDisabled()} style={cancelOptions.style} className={cancelOptions.className} />;
        }
    ...
    renderBasic() {
        ...
        const buttonClassName = classNames('p-button p-component p-fileupload-choose', { 'p-fileupload-choose-selected': this.hasFiles(), 'p-disabled': this.props.disabled, 'p-focus': this.state.focused } as any, chooseOptions.className);
        const iconClassName = classNames('p-button-icon p-button-icon-left pi', { 'pi-plus': !chooseOptions.icon && (!this.hasFiles() || this.props.auto), 'pi-upload': !chooseOptions.icon && this.hasFiles() && !this.props.auto } as any, chooseOptions.icon);
        const labelClassName = 'p-button-label p-clickable';
