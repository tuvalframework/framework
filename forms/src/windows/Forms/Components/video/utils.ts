...
const requests = {}
export function getSDK (url, sdkGlobal, sdkReady: any = null, isLoaded = (e) => true, fetchScript: any = loadScript) {
  const existingGlobal: any = getGlobal(sdkGlobal)
  ...
  return new Promise((resolve, reject) => {
    ...
    if (sdkReady) {
      const previousOnReady:any = (window as any)[sdkReady];
...
export function getConfig (props, defaultProps) {
  return deepmerge(defaultProps.config, props.config)
}
...
export function omit (object, ...arrays) {
  ...
  for (const key of keys) {
    if ((omitKeys as any).indexOf(key) === -1) {
...
export function supportsWebKitPresentationMode (video = document.createElement('video')) {
  ...
  return (video as any).webkitSupportsPresentationMode && typeof (video as any).webkitSetPresentationMode === 'function' && notMobile
}