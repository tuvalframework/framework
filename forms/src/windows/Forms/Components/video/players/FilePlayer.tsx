...
const IS_IOS = HAS_NAVIGATOR && (/iPad|iPhone|iPod/.test(navigator.userAgent) || IS_IPAD_PRO) && !(window as any).MSStream
...
export  class FilePlayer extends React.Component {
  ...
  enablePIP () {
    if (this.player.requestPictureInPicture && (document as any).pictureInPictureElement !== this.player) {
  ...
  disablePIP () {
    if ((document as any).exitPictureInPicture && (document as any).pictureInPictureElement === this.player) {
