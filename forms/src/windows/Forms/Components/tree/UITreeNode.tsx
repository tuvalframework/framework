...
export class UITreeNode extends React.Component {
    ...
    contentElement(contentElement: any, arg1: string) {
    ...
    renderCheckbox() {
        if (this.isCheckboxSelectionMode() && this.props.node.selectable !== false) {
            ...
            const className = classNames('p-checkbox-box', { 'p-highlight': checked, 'p-indeterminate': partialChecked, 'p-disabled': this.props.disabled } as any);
            const icon = classNames('p-checkbox-icon p-c', { 'pi pi-check': checked, 'pi pi-minus': partialChecked } as any);
    ...
    renderToggler(expanded) {
        ...
        //const iconClassName = classNames('p-tree-toggler-icon pi pi-fw', { 'pi-chevron-right': !expanded, 'pi-chevron-down': expanded } as any);
        const iconClassName = classNames('p-tree-toggler-icon pi pi-fw', { 'pi-plus': !expanded, 'pi-minus': expanded } as any);
        let content = (
    ...
    renderNode() {
        const className = classNames('p-treenode', { 'p-treenode-leaf': this.isLeaf() } as any, this.props.node.className)
