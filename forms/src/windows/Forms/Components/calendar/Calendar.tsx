...
import React, { createElement, createRef, Fragment, Portal } from '../../../../preact/compat';
...
import { ConnectedOverlayScrollHandler, mask } from '../utils/Utils';
...
export class Calendar extends React.Component {
    ...
    inputRef: any;
    tooltip: any;
    viewStateChanged: any;
    hideTimeout: any;
    touchUIMask: any;
    scrollHandler: any;
    ignoreFocusFunctionality: any;
    isKeydown: boolean;
    ...
    initFocusableCell() {
        ...
        if (this.view === 'month') {
            ...
            cells.forEach(cell => (cell as any).tabIndex = -1);
    ...
    navigateToMonth(prev, groupIndex, event) {
        if (prev) {
            if (this.props.numberOfMonths === 1 || (groupIndex === 0)) {
                ...
                let focusCell: any = cells[cells.length - 1];
                focusCell.tabIndex = '0';
                focusCell.focus();
            }
        }
    ...
    onDateSelect(event, dateMeta, timeMeta?) {
        ...
        DomHandler.find(this.overlayRef.current, '.p-datepicker-calendar td span:not(.p-disabled)').forEach(cell => (cell as any).tabIndex = -1);
    ...
    renderWeek(weekDates, weekNumber, groupIndex) {
        const week = weekDates.map((date) => {
            ...
            const cellClassName = classNames({ 'p-datepicker-other-month': date.otherMonth, 'p-datepicker-today': date.today } as any);
            const dateClassName = classNames({ 'p-highlight': selected, 'p-disabled': !date.selectable } as any);
    ...
    renderMonthViewMonth(index) {
        const className = classNames('p-monthpicker-month', { 'p-highlight': this.isMonthSelected(index) } as any);
    ...
    renderButton() {
        if (this.props.showIcon) {
            return (
                <ButtonComponent type="button" icon={this.props.icon} onClick={this.onButtonClick} tabIndex="-1"
        ...
        if (this.props.showButtonBar) {
            ...
            return (
                <div className="p-datepicker-buttonbar">
                    <ButtonComponent type="button" label={today} onClick={this.onTodayButtonClick} onKeyDown={e => this.onContainerButtonKeydown(e)} className={todayClassName} />
                    <ButtonComponent type="button" label={clear} onClick={this.onClearButtonClick} onKeyDown={e => this.onContainerButtonKeydown(e)} className={clearClassName} />
                </div>
            );
        }
