...
export class TableBody extends React.Component {
    ...
    rangeRowIndex: any;
    anchorRowFirst: any;
    rowTouched: boolean = false;
    anchorCellIndex: any;
    dragSelectionHelper: any;
    initialDragPosition: any;
    compareSelectionBy: any;
    rowDragging: any;
    draggedRowIndex: any;
    droppedRowIndex: any;
    ...
    onRangeSelection(event) {
        ...
        let selection = this.isMultipleSelection() ? [...(new Set([...(this.props.selection || []), ...selectionInRange])) as any] : selectionInRange;
    ...
    selectRangeOnCell(event, rowRangeStart, rowRangeEnd) {
        ...
        for (let i = rowRangeStart; i <= rowRangeEnd; i++) {
            let rowData = value[i];
            let columns = React.Children.toArray(this.getChildren()/* this.props.children */);
    ...
    isSelectionEnabled() {
        if (this.props.selectionMode || this.props.selectionModeInColumn != null) {
            ...
            if (Array.isArray(this.getChildren())) {
                for (let i = 0; i < this.getChildren().length; i++) {
                    if (this.getChildren()[i].props.selectionMode) {
                        return true;
                    }
                }
            }
    ...
    renderRowGroupHeader(rowData, index) {
        ...
        return (
            <tr role="row" key={index + '_rowgroupheader'} className="p-rowgroup-header">
                <td role="cell" colSpan={React.Children.count(this.getChildren())}>
                    {content}
            ...
            if (this.props.value && this.props.value.length) {
                ...
                for (let i = startIndex; i < endIndex; i++) {
                    ...
                    if (expanded && !(hasSubheaderGrouping && this.props.expandableRowGroups)) {
                        ...
                        let expandedRow = <tr key={id} id={id} role="row" className="p-row-expanded"><td role="cell" colSpan={this.getChildren().length}>{expandedRowContent}</td></tr>
                ...
                rows = !this.props.loading && emptyMessage !== null ?
                    <tr role="row" className="p-datatable-emptymessage">
                        <td role="cell" colSpan={this.getChildren().length}>
                            {
