...
export class SketchFont {
  ...
  cache: {};
  font: any;
  public constructor(p: SketchGraphics) {
    this.parent = p;
  ...
  public textBounds(str, x, y, fontSize, options) {
    x = x !== undefined ? x : 0;
    y = y !== undefined ? y : 0;
  ...
  public textToPoints(txt, x, y, fontSize, options) {
    let xoff: any = 0,
      result: any[] = [],
    ...
    for (var i = 0; i < glyphs.length; i++) {
      if (!isSpace(i)) {
        ...
        for (let j = 0; j < paths.length; j++) {
          const pts: any[] = pathToPoints(paths[j], options);
  ...
  public _getGlyphs(str) {
    return this.font.stringToGlyphs(str);
  }

  public _getPath(line, x, y, options) {
    var p =
      (options && options.renderer && options.renderer._pInst) || this.parent,
      renderer = p._renderer,
  ...
  public _getPathData(line, x, y, options) {
    var decimals = 3;
  ...
  public _getSVG(line, x, y, options) {
    var decimals = 3;
  ...
  public _renderPath(line, x, y, options) {
    var pdata,
      pg = (options && options.renderer) || this.parent.renderer,
  ...
  public _textWidth(str, fontSize) {
    return this.font.getAdvanceWidth(str, fontSize);
  }

  public _textAscent(fontSize) {
    return this.font.ascender * this._scale(fontSize);
  }

  public _textDescent(fontSize) {
    return -this.font.descender * this._scale(fontSize);
  }

  public _scale(fontSize) {
    return (
      1 / this.font.unitsPerEm * (fontSize || this.parent.renderer._textSize)
    );
  }

  public _handleAlignment(renderer, line, x, y, textWidth?) {
...
function collinear(a, b, c, thresholdAngle) {
  ...
  if (typeof (<any>collinear).tmpPoint1 === 'undefined') {
    (<any>collinear).tmpPoint1 = [];
    (<any>collinear).tmpPoint2 = [];
  }

  var ab = (<any>collinear).tmpPoint1,
    bc = (<any>collinear).tmpPoint2;
  ab.x = b.x - a.x;
  ab.y = b.y - a.y;
  bc.x = c.x - b.x;
  bc.y = c.y - b.y;
...
function path2curve(path, path2?) {
  ...
  var pcoms1: any[] = []; // path commands of original path p
  var pcoms2: any[] = []; // path commands of original path p2
  var ii;

  var processPath = function (path: any[], d, pcom) {
    var nx,
      ny,
      tq = { T: 1, Q: 1 };
    if (!path) {
...
function a2c(x1: number, y1: number, rx: number, ry: number, angle, lac, sweep_flag, x2, y2, recursive) {
  ...
  if (!recursive) {
    ...
    f1 = Math.asin((((y1 - cy) / ry) as any).toFixed(9));
    f2 = Math.asin((((y2 - cy) / ry) as any).toFixed(9));
